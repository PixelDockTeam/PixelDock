...
async fn torrent_create_from_url(
    ...
    api.api_add_torrent(AddTorrent::Url(url.into()), opts).await
}
...
#[tauri::command]
async fn launch_exe(exePath: String) -> String {
    let exe_path_buf = PathBuf::from(&exePath);
    let game_dir_path = exe_path_buf.parent().expect("Failed to get game directory");
    let start_time = SystemTime::now()
        .duration_since(UNIX_EPOCH)
        .expect("Time went backwards")
        .as_secs();
    // let game_task = tokio::spawn(async move {});
    // let success = game_task.await.unwrap();
    ...
    let end_time = SystemTime::now()
        ...
        .expect("Time went backwards")
        .as_secs();
    let playtime = end_time - start_time;
    println!("Playtime for {} seconds", playtime);

    "success".to_string()
    // if output.status.success() {
    //     "EXE launched successfully".to_string()
    // } else {
    //     "Failed to launch EXE".to_string()
    // }
}
